...
class energy_arbitrage_stochastic_optimizer:
    def __init__(self, name, risk_factor, objective_function):
        self.name: str = name
        self.risk_factor = risk_factor
        self.objective_function = objective_function

    def optimize_daa(
        self, n_cycles: int, energy_cap: int, power_cap: int, quantile_forecasts
    ):
        """
        ...
        def soc_step_constraint(model, q):
            ...
            return (
                model.soc[q + 1]
                == model.soc[q]
                + power_cap / 4 * model.cha_daa[q]
                - power_cap / 4 * model.dis_daa[q]
            )
        ...
        def charge_cycle_limit(model):
            ...
            return (
                sum(model.cha_daa[q] * power_cap / 4 for q in model.Q) <= volume_limit
            )
        ...
        def discharge_cycle_limit(model):
            ...
            return (
                sum(model.dis_daa[q] * power_cap / 4 for q in model.Q) <= volume_limit
            )
        ...
        model.obj = pyo.Objective(
            expr=pyo.quicksum(
                ...
                * (
                    ...
                    - self.risk_factor
                    * (daa_price_vector_05[q - 1] - daa_price_vector_01[q - 1])
                    * model.cha_daa[q]
                    - self.risk_factor
                    * (daa_price_vector_09[q - 1] - daa_price_vector_05[q - 1])
                    * model.dis_daa[q]
                )
                for q in model.Q
            ),
    ...
    def optimize_ida(
        ...
        def soc_step_constraint(model, q):
            ...
            return model.soc[q + 1] == model.soc[q] + power_cap / 4 * (
                model.cha_ida[q]
                - model.dis_ida[q]
        ...
        def charge_cycle_limit(model):
            ...
            return (
                np.sum(step1_cha_daa)
                + sum(model.cha_ida[q] for q in model.Q)
                - sum(model.dis_ida_close[q] for q in model.Q)
        ...
        def discharge_cycle_limit(model):
            ...
            return (
                np.sum(step1_dis_daa)
                + sum(model.dis_ida[q] for q in model.Q)
                - sum(model.cha_ida_close[q] for q in model.Q)
            ) * power_cap / 4 <= volume_limit
            ...
            expr=sum(
                ...
                * (
                    ...
                    * (
                    ...
                    - self.risk_factor
                    * (ida_price_vector_05[q - 1] - ida_price_vector_01[q - 1])
                    * model.cha_ida[q]
                    - self.risk_factor
                    * (ida_price_vector_09[q - 1] - ida_price_vector_05[q - 1])
                    * model.dis_ida[q]
                )
                for q in model.Q
            ),
        ...
        step2_soc_ida = [
            model.soc[q].value for q in range(1, len(ida_price_vector_01) + 1)
        ]
        step2_cha_ida = [
            model.cha_ida[q].value for q in range(1, len(ida_price_vector_01) + 1)
        ]
        step2_dis_ida = [
            model.dis_ida[q].value for q in range(1, len(ida_price_vector_01) + 1)
        ]
        step2_cha_ida_close = [
            model.cha_ida_close[q].value for q in range(1, len(ida_price_vector_01) + 1)
        ]
        step2_dis_ida_close = [
            model.dis_ida_close[q].value for q in range(1, len(ida_price_vector_01) + 1)
        ]
        ...
        step2_cha_daaida = (
            np.asarray(step1_cha_daa) - step2_dis_ida_close + step2_cha_ida
        )

        assert type(step1_cha_daa) == type(step2_dis_ida_close)
        step2_dis_daaida = (
            np.asarray(step1_dis_daa) - step2_cha_ida_close + step2_dis_ida
        )
    ...
    def optimize_idc(
        ...
        energy_cap: int,
        power_cap: int,
        step2_cha_daaida: list,
        step2_dis_daaida: list,
        quantile_forecasts,
    ):
        """
        ...
        def soc_step_constraint(model, q):
            ...
            return model.soc[q + 1] == model.soc[q] + power_cap / 4 * (
                model.cha_idc[q]
                - model.dis_idc[q]
                + model.cha_idc_close[q]
                - model.dis_idc_close[q]
                + step2_cha_daaida[q - 1]
                - step2_dis_daaida[q - 1]
            )
        ...
        def charge_cycle_limit(model):
            ...
            return (
                np.sum(step2_dis_daaida)
                + sum(model.dis_idc[q] for q in model.Q)
                - sum(model.cha_idc_close[q] for q in model.Q)
            ) * power_cap / 4 <= volume_limit
        ...
        def discharge_cycle_limit(model):
            ...
            return (
                np.sum(step2_cha_daaida)
                + sum(model.cha_idc[q] for q in model.Q)
                - sum(model.dis_idc_close[q] for q in model.Q)
            ) * power_cap / 4 <= volume_limit
                ...
                [
                    ...
                    * (
                        ...
                        * (
                        ...
                        - self.risk_factor
                        * (idc_price_vector_05[q - 1] - idc_price_vector_01[q - 1])
                        * model.cha_idc[q]
                        - self.risk_factor
                        * (idc_price_vector_09[q - 1] - idc_price_vector_05[q - 1])
                        * model.dis_idc[q]
                    )
                    for q in model.Q
                ]
        ...
        # Retrieve arrays of resulting optimal soc/charge/discharge schedules after the ID Auction:
        step3_soc_idc = [
            model.soc[q].value for q in range(1, len(idc_price_vector_01) + 1)
        ]
        step3_cha_idc = [
            model.cha_idc[q].value for q in range(1, len(idc_price_vector_01) + 1)
        ]
        step3_dis_idc = [
            model.dis_idc[q].value for q in range(1, len(idc_price_vector_01) + 1)
        ]
        step3_cha_idc_close = [
            model.cha_idc_close[q].value for q in range(1, len(idc_price_vector_01) + 1)
        ]
        step3_dis_idc_close = [
            model.dis_idc_close[q].value for q in range(1, len(idc_price_vector_01) + 1)
        ]
        ...
        step3_cha_daaidaidc = (
            np.asarray(step2_cha_daaida) - step3_dis_idc_close + step3_cha_idc
        )
        step3_dis_daaidaidc = (
            np.asarray(step2_dis_daaida) - step3_cha_idc_close + step3_dis_idc
        )
