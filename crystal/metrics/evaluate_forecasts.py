...
def compute_metrics(ground_truth: pd.DataFrame, forecasts: pd.DataFrame, prediction_length: int) -> pd.DataFrame:
    """
    ...
    if len(ground_truth) < prediction_length or len(forecasts) < prediction_length:
        raise ValueError("Insufficient data for evaluation. Ensure the ground truth and forecast cover the prediction length.")

    # Align ground truth and forecast by timestamp
    merged = pd.merge(ground_truth, forecasts, on='timestamp', suffixes=('_actual', '_forecast'))

    # Check for alignment issues
    if merged.empty:
        raise ValueError("No common timestamps found between ground truth and forecasts. Ensure proper alignment.")
    ...
    for quantile in np.arange(0.1, 1, 0.1):
        ...
        metrics[f'Pinball Loss {quantile}'] = sklearn.metrics.mean_pinball_loss(y_actual, forecasts[str(round(quantile, 2))], alpha=quantile)
